...
// Initialize Groq
const GROQ_API_KEY = process.env.GROQ_API_KEY || 'dummy_key_placeholder';
...
app.post('/api/chat', async (req, res) => {
  ...
  try {
    ...
    const parsedText = content.trim().split('. ').join('.\n');
    res.json({ reply: parsedText });
  } catch (error) {
    console.error('Groq API Error:', error.message || 'Error occurred');
    res.json({ reply: "API Error: Unable to fetch generation from Groq." });
  }
});
