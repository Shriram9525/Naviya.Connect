  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="card" style={{ maxWidth: '800px', margin: '40px auto', minHeight: '600px', padding: '40px' }}>
      <div style={{ textAlign: 'center', marginBottom: '40px' }}>
        ...
        <h2 className="boldonse-regular" style={{ fontSize: '2.5rem', marginBottom: '8px', color: 'var(--primary)' }}>Paragraph Generator</h2>
        <p style={{ color: 'var(--text-muted)', fontSize: '1.2rem' }}>Powered by Groq API Llama 3.1</p>
      </div>
      ...
      {result && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} style={{ background: 'var(--surface)', padding: '32px', borderRadius: '16px', border: '1px solid rgba(11,60,93,0.1)', boxShadow: '0 4px 15px rgba(0,0,0,0.05)' }}>
          <h3 style={{ marginBottom: '16px', color: 'var(--primary)' }}>Generated Output:</h3>
          <div style={{ fontSize: '1.15rem', lineHeight: '1.8', color: 'var(--text-main)', whiteSpace: 'pre-wrap' }}>
            {result}
          </div>
        </motion.div>
      )}
      ...
      {!result && !loading && !error && (
        <div style={{ textAlign: 'center', color: 'var(--text-muted)', marginTop: '60px', padding: '40px', border: '2px dashed rgba(11,60,93,0.2)', borderRadius: '16px' }}>
          <p>Your Groq generation results will appear here in raw paragraph format.</p>
        </div>
      )}
    </motion.div>
  );
